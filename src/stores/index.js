...
const store = createStore({
  ...
  actions: {
    async updateDatosProducto({ commit }, codigoOF) {
      ...
      try {
        const response = await axios.get(`http://192.168.1.33:8000/productos/of/${codigoOF}`);
    ...
    async registrarLecturaCalidad({ commit }, codigoOF) {
      ...
      commit('setError', null);
      try {
        const response = await axios.put(`http://192.168.1.33:8000/productos/of/${codigoOF}/calidad`);
        commit('setDatosProducto', response.data);
      } catch (error) {
